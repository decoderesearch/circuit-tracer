...
class NNSightReplacementModel(LanguageModel):
    ...
    def ensure_tokenized(self, prompt: str | torch.Tensor | list[int]) -> torch.Tensor:
        ...
        if gemma_3_it:
            ...
            tokenization_error = "Input tokens should start with <bos><start_of_turn>user\n, but got {tokens}"
            assert tokens.size(0) >= 4 and torch.all(tokens[:4] == ignore_prefix), tokenization_error.format(tokens=self.tokenizer.decode(tokens.cpu().tolist()))
            return tokens
    ...
    def setup_attribution(self, inputs: str | torch.Tensor):
        ...
        attribution_data = transcoders.compute_attribution_components(mlp_in_cache, self.zero_positions)  # type: ignore

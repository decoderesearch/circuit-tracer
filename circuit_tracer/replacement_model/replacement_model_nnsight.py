...
class NNSightReplacementModel(LanguageModel):
    ...
    def get_activation_fn(
        ...
        def fetch_activations(
            ...
            layers = range(self.cfg.n_layers) if activation_layers is None else activation_layers  # type:ignore
            ...
            for layer in layers:
                ...
                if barrier and barrier_layers is not None and layer in barrier_layers:
                    barrier()
    ...
    def get_activations(
        ...
        with torch.inference_mode(), self.trace(inputs):
            ...
            logits = logits.save()  # type: ignore
            activation_cache = activation_cache.save()  # type: ignore
    ...
    def setup_attribution(self, inputs: str | torch.Tensor):
        ...
        with self.trace(tokens):
            ...
            mlp_in_cache = torch.cat(mlp_in_cache, dim=0).save()  # type: ignore
            mlp_out_cache = torch.cat(mlp_out_cache, dim=0).save()  # type: ignore
            logits = self.output.logits.save()
        ...
        error_vectors[:, 0] = 0
        token_vectors = self.embed_weight[tokens].detach()  # (n_pos, d_model)  # type: ignore
    ...
    def setup_intervention_with_freeze(
        ...
        with self.trace() as tracer:
            ...
            dict_to_freeze = get_locs_to_freeze().save()  # type: ignore
            for freeze_loc_name, loc_type_to_freeze in get_locs_to_freeze().items():
                with tracer.invoke():
                    for layer, loc_to_freeze in enumerate(loc_type_to_freeze):
                        ...
                        cache[freeze_loc_name, layer] = freeze_loc_output.save()  # type: ignore
        ...
        for layer in range(self.cfg.n_layers):
            ...
            if constrained_layers is None or layer in constrained_layers:
                if direct_effects_barrier:
                    direct_effects_barrier()
                transcoder_output_loc = self.get_feature_output_loc(layer)
                transcoder_output_loc.output = transcoder_output_loc.output + layer_deltas[layer]  # type: ignore
                layer_deltas[layer] *= 0

        return self.output.logits.save()
    ...
    def feature_intervention_generate(
        ...
        **kwargs,
        ...
        activation_matrix, activation_fn = self.get_activation_fn(
            apply_activation_function=apply_activation_function, append=True, sparse=sparse
        )
        ...
        with self.generate(**kwargs) as tracer:
            ...
            with tracer.invoke():
                with tracer.iter[:] as act_idx:
                    ...
                    activation_cache[0] = iter_activation_cache.save()
            ...
            all_logits = list().save()  # type: ignore
            with tracer.invoke():
            ...
            with tracer.invoke():
                out = self.generator.output.save()

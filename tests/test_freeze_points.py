...
MODEL_CONFIGS_NNSIGHT = [
    ("google/gemma-2-2b", "gemma"),
    ("google/gemma-3-1b-pt", "mwhanna/gemma-scope-2-1b-pt/clt/width_262080_affine_l0_small"),
    (
    ...
    def get_gradients_with_freeze(tokens: torch.Tensor, freeze_hooks):
        ...
        with model.trace() as tracer:  # type: ignore
            ...
            with tracer.invoke():
                ...
                max_logit = logits[0, -1].max().save()  # type: ignore

        mlp_out_grads = []
        with max_logit.backward():  # type: ignore
            for layer in reversed(range(n_layers)):
                mlp_out_grads.append(mlp_outs[layer].grad.save())  # type: ignore
            embed_grad = embed_out.grad.save()  # type: ignore
